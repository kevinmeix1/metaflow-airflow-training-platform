...
This project treats generated training and backfill reports as build subjects. The CI workflow uploads them as review artifacts and signs their provenance with GitHub artifact attestations.
...
The Sigstore policy-controller manifest opts the `mlops-training` namespace into verification and starts with `no-match-policy: warn`. Production rollout should move to deny mode only after every training image has digest-pinned signatures and provenance attestations.
...
gh attestation verify .local/reports --repo kevinmeix1/metaflow-airflow-training-platform
cosign verify-attestation --type slsaprovenance ghcr.io/kevinmeix1/metaflow-airflow-training-platform@sha256:<digest>
```

## Production Notes

In a real training platform, the same pattern should be applied to Airflow images, training images, model artifacts, dataset manifests, SBOMs, and backfill bundles. The next hardening step would add image builds, SBOM generation, vulnerability scan attestations, digest-pinned workload manifests, and scheduled OpenSSF Scorecard-style repository posture checks.
