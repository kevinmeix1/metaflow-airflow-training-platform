...
def build_orchestration_scorecard(
    ...
    *,
    ...
    checks = [
        ...
        ("opencost_finops", _present(content, "cost_observability_report.json", "OpenCost", "node_gpu_hourly_cost") and _present(content, "TrainingBackfillCostBudgetExceeded"), "OpenCost and Prometheus budget alerts attribute daily, backfill, distributed, recovery, GPU, and PVC artifact spend"),
        ("kueue_elastic_workloads", _present(content, "elastic_workload_plan.json", "ElasticJobsViaWorkloadSlices") and _present(content, "workload-slice-name", "JobSet"), "Kueue Workload Slices and JobSet support elastic backfill scale-up, scale-down, and recovery preemption"),
        ...
        ("checkpointed_distributed_training", _present(content, "checkpoint_training_readiness_plan.json", "metaflow_checkpoint_restore_seconds", "JobSet") and _present(content, "METAFLOW_CHECKPOINT_SCOPE", "checkpointed-training"), "Metaflow checkpoint recovery is tied to JobSet replica groups, Kueue queues, resume SLAs, and checkpoint artifact budgets"),
        ...
        ("airflow_asset_partitioning", _present(content, "asset_partitioning_plan.json", "PartitionedAssetTimetable", "CronPartitionTimetable") and _present(content, "dag_run.partition_key", "StartOfHourMapper"), "Airflow 3.2 asset partitioning scopes dataset snapshots, Metaflow runs, evaluation gates, and model registration to aligned partitions"),
        ("airflow_stateful_orchestration", all(token in content for token in ["airflow_stateful_orchestration_plan.json", "task_state_store", "asset_state_store", "RollupMapper", "FanOutMapper", "PartitionedAtRuntime", "apache-airflow==3.3.0", "airflow-sdk-contract"]), "Airflow 3.3 training DAGs preserve external run identity with scoped state stores, bounded partition mapping, and a real SDK parse gate"),
        ("airflow_multi_team_readiness", _present(content, "multi_team_readiness_plan.json", "team_name", "multi_team = True") and _present(content, "AssetAccessControl", "airflow triggerer --team-name"), "Airflow multi-team preview readiness isolates training DAG bundles, pools, triggerers, secrets, executors, and asset events"),
        ...
        ("pod_resource_envelopes", _present(content, "pod_resource_envelope_plan.json", "PodLevelResources", "schedulingGates") and _present(content, "scheduler_pending_pods", "PodSchedulingReadiness"), "Kubernetes pod-level resource envelopes and scheduling gates avoid training backfill scheduler churn before prerequisites are ready"),
        ("kueue_cohort_fair_sharing", _present(content, "cohort_fair_sharing_plan.json", "AdmissionFairSharing", "preemptionStrategies") and _present(content, "borrowingLimit", "lendingLimit", "fairSharing"), "Kueue Fair Sharing and Admission Fair Sharing protect production backfills while exploration borrows idle quota"),
        ("kueue_flavor_fungibility", _present(content, "flavor_fungibility_plan.json", "flavorFungibility", "TryNextFlavor") and _present(content, "BorrowingOverPreemption", "ResourceFlavor"), "Kueue ResourceFlavor fallback avoids premature borrowing or preemption across production, replay, and HPO pools"),
        ("kueue_pending_workload_visibility", _present(content, "pending_workload_visibility_plan.json", "VisibilityOnDemand", "pendingworkloads") and _present(content, "kueue_admission_wait_time_seconds", "kueue_cluster_queue_resource_pending"), "Kueue VisibilityOnDemand exposes pending backfill, replay, and HPO workloads before Airflow widens fanout"),
        ("kubernetes_workload_aware_scheduling", _present(content, "workload_aware_scheduling_plan.json", "scheduling.k8s.io/v1alpha2", "kind: PodGroup") and _present(content, "WorkloadWithJob", "completionMode: Indexed", "ResourceClaimTemplate"), "Kubernetes v1.36 Workload and PodGroup APIs prepare partition backfills, HPO sweeps, and failed-partition replay jobs for atomic gang scheduling, topology constraints, DRA ResourceClaims, and workload-aware preemption"),
        ("runtime_security_userns_kubelet_authz", _present(content, "runtime_security_plan.json", "hostUsers: false", "KubeletFineGrainedAuthz") and _present(content, "nodes/metrics", "nodes/stats", "ValidatingAdmissionPolicy"), "Kubernetes v1.36 user namespaces and fine-grained kubelet authorization reduce host and kubelet blast radius for Metaflow training telemetry"),
        ("control_plane_freshness_diagnostics", _present(content, "control_plane_diagnostics_plan.json", "/statusz", "/flagz") and _present(content, "TrainingBackfillControllerCacheStale", "KubeletPSIMemoryStallHigh", "NativeHistogramMetrics"), "Kubernetes v1.36 controller staleness, ComponentStatusz, ComponentFlagz, PSI, and native histogram readiness protect training backfills from stale control-plane state"),
        ("memory_qos_tiered_protection", _present(content, "memory_qos_plan.json", "MemoryQoS", "TieredReservation") and _present(content, "memory.high", "memory.low", "TrainingMemoryQoSPSIPressureHigh"), "Kubernetes v1.36 Memory QoS tiered protection protects Airflow and MLflow control tasks while large training fanout yields under memory pressure"),
        ("hpa_scale_to_zero_external_metrics", _present(content, "hpa_scale_to_zero_plan.json", "HPAScaleToZero", "minReplicas: 0") and _present(content, "type: External", "type: Object", "TrainingScaleToZeroWakeupFailed"), "Kubernetes v1.36 HPA scale-to-zero is limited to elastic training workers with Object or External wake metrics and explicit cold-start budgets"),
        ("suspended_job_resource_mutation", _present(content, "suspended_job_resources_plan.json", "MutablePodResourcesForSuspendedJobs", "suspend: true") and _present(content, "ValidatingAdmissionPolicy", "TrainingSuspendedJobResizeStale"), "Kubernetes v1.36 suspended Job resource mutation right-sizes queued training Jobs before unsuspend without rewriting Airflow or MLflow control probes"),
        ("constrained_impersonation_least_privilege", _present(content, "constrained_impersonation_plan.json", "ConstrainedImpersonation", "impersonate:serviceaccount") and _present(content, "impersonate-on:serviceaccount:get", "TrainingConstrainedImpersonationAuditMissing"), "Kubernetes v1.36 constrained impersonation splits Airflow support identity grants from scoped impersonated actions"),
        ("dra_resource_health_status", _present(content, "resource_health_status_plan.json", "ResourceHealthStatus", "allocatedResourcesStatus") and _present(content, "DeviceTaintRule", "kube_resourceclaim_status_devices"), "Kubernetes v1.36 DRA ResourceHealthStatus, ResourceClaim device status, and DeviceTaintRule quarantine protect partitioned training"),
        ("dra_advanced_device_sharing", _present(content, "advanced_device_sharing_plan.json", "DRAPrioritizedList", "DRAPartitionableDevices") and _present(content, "DRAConsumableCapacity", "DRADeviceBindingConditions"), "DRA prioritized alternatives, partitionable devices, consumable capacity, and binding conditions reduce training accelerator waste"),
        ("dra_admin_access_diagnostics", _present(content, "admin_access_diagnostics_plan.json", "DRAAdminAccess", "adminAccess: true") and _present(content, "resource.kubernetes.io/admin-access", "mlops-training-dra-admin"), "Kubernetes v1.36 DRA AdminAccess diagnostics inspect in-use training devices while deterministic replay stays runnable"),
        ("kubernetes_inplace_resize", _present(content, "inplace_resize_plan.json", "InPlaceOrRecreate", "PodResizePending") and _present(content, "pods/resize", "PodResizeInProgress"), "Kubernetes in-place Pod resize and pod-level resource resize preserve partition lineage during non-disruptive training scaling"),
    ...
    report = {
        ...
        "research_basis": [
            ...
            "OpenCost exporter metrics for training partition, backfill wave, GPU, retry, and artifact storage cost allocation",
            "Kueue Elastic Workloads with Workload Slices and JobSet integration for dynamic backfill scale changes",
            ...
            "Metaflow checkpointing with task-scoped recovery combined with Kueue-scheduled JobSet distributed training",
            "Airflow 3 DAG Bundles and DAG versioning for reproducible training backfills and failed-partition replay",
            "Airflow 3.2 asset partitioning with PartitionedAssetTimetable for partition-aware Metaflow training backfills",
            "Airflow multi-team preview mode for training-owned DAG Bundles, team-scoped resources, triggerers, executors, and asset-event filtering",
            "Airflow 3 AssetWatchers, BaseEventTrigger compatibility, shared-stream polling, and conditional training asset expressions",
            "Kubernetes PodLevelResources and Pod Scheduling Readiness gates for scheduler-efficient training backfills",
            "Kueue Fair Sharing and Admission Fair Sharing for training cohort scheduling fairness",
            "Kueue FlavorFungibility for ResourceFlavor fallback before borrowing or preempting partitioned training capacity",
            "Kueue VisibilityOnDemand pending workload APIs for backfill, schema replay, failed-partition recovery, and HPO triage",
            "Kubernetes v1.36 HPA scale-to-zero for Object or External metrics with HPAScaleToZero feature gate and Airflow control-plane guardrails",
            "Kubernetes v1.36 MutablePodResourcesForSuspendedJobs for queue-controller training resource patching before unsuspend",
            "Kubernetes v1.36 ConstrainedImpersonation with separate identity and impersonate-on action authorization for least-privilege Airflow and backfill support",
            ...
            "Kubernetes v1.36 user namespaces GA with pod.spec.hostUsers=false and fine-grained kubelet API authorization GA using nodes/metrics, nodes/stats, nodes/log, and nodes/pods instead of nodes/proxy",
            ...
            "Kubernetes v1.36 DRA ResourceHealthStatus, ResourceClaim status.devices, and DeviceTaintRule quarantine for training accelerator incidents",
            "Kubernetes DRA prioritized alternatives, partitionable devices, consumable capacity, and device binding conditions for training accelerator efficiency",
            "Kubernetes v1.36 DRA AdminAccess for namespace-scoped backfill and HPO diagnostics on devices already in use",
            "Kubernetes v1.35 in-place Pod Resize and v1.36 pod-level resource resize for non-disruptive training backfill scaling",
